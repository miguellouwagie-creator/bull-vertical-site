...
export const Navigation = () => {
  ...
  useEffect(() => {
    ...
    const ids = ["services", "safety", "faq", "contact"];
    const sections = ids
    ...
    const io = new IntersectionObserver(
      (entries) => {
        ...
        if (inView.length > 0) {
          setActiveId((inView[0].target as HTMLElement).id);
        }
      },
            ...
            <ul className="flex items-center gap-1">
              {navItems.map((item) => (
                <li key={item.href}>
                  <a
                    href={item.href}
                    // FIX #6: aria-current must be "page" (not "true") for
                    // screen readers to announce the active nav item correctly.
                    aria-current={isActiveHref(item.href) ? "page" : undefined}
                    className={[
                      "px-3 py-2 text-sm font-semibold rounded-full transition relative",
                      "text-foreground/80 hover:text-foreground hover:bg-foreground/5",
                      isActiveHref(item.href) ? "bg-foreground/10 text-foreground" : "",
